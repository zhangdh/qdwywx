...
function saveSuggestion(){
	...
	var postData = "suggestion="+$("#suggestion").val();
	sys_ajaxPost("/cc/app/help.do?method=saveSuggestion",postData,function(json){
		if(!json.login){
			alert("状态验证失败，请重新登录");
			sys_goUrl("../login.html")
		}else if(json.result){
			alertMsg(json);
			$.closeModal(".popup-suggestion");
		}else{
			alertMsg(json);
		}
	});
